...
class GPTNavAgent(BaseAgent):
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        for t in range(self.args.max_action_len):
            ...
            normal=self.spatialexpert.spatial_representation(obs, t, self.env) 
            if normal==1:
                # print('[calling spatial_reasoning]')
                s_t=self.spatialexpert.spatial_reasoning(obs, t) 
            elif normal==2:  #alternative path to stop point
                s_t=self.spatialexpert.next_viewpoint_to_stop
